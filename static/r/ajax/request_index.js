...
function lysavechange(num){
    ...
    var f=function(res){
        ...
        if(response['status'] == trues){
            ...
            if(response['data']['result'] != ''){
                $.each(response['data']['result'],function(k,v){
                  ...
                  switch(v['goods_source']){
                    ...
                    content +='<a href="goodsinfo?id='+v['goods_id']+'" class="slista">'+
                            '<div class="spimg">'+
                            '<img class="shop_img" src="'+v['goods_img']+'">'+  
                            '<span class="shop_by">包邮</span></div>'+
                            '<div class="shop_mesg">'+
                            '<span class="shopName c_shopn">'+v['goods_name']+'</span>'+
                            '<span class="shopprice c_shopp"><b class="jdColor">'+v['goods_integral']/100+'</b>金点 / <b class="pirceM">'+v['goods_price']/100+'</b>元</span>'+
                            '<p class="shopdh_change">'+sous+'兑换价：<b class="sjf">'+v['goods_source_info']['credit']+'</b>积分 </p>'+
							'<p class="shopdh_change">市场价: <b class="jspric">'+v['goods_source_info']['price']/100+'</b>元'+
                            '</p></div></a>';
                })
            }else{
               content = '';
            ...
            //判断当前页数
            if(response['data']['Countpage'] == 0 && num == 0){
            	$('.noh_box').show();
                $('.nohave_p').html('目前没有相关商品~');
                $(window).unbind('scroll');    //解禁window的scroll事件
            }else if(response['data']['Countpage'] == 0 && pagenum > 0){
                $('.nohave_p').html('到底了~');
                $(window).unbind('scroll');    //解禁window的scroll事件
